...
const SendAddressInfoEditor = props => {
    const { sendAddressInfoEditorReducer: { saveSendAddressInfo: { isResultStatus } } } = props
    return (
        <Container>
            <Content>
                <Field
...
const mapStateToProps = (state, ownProps) => {
    ...
    return {
        ...
        sendAddressInfoEditorReducer: state.sendAddressInfoEditorReducer
    }
}
...
export default connect(mapStateToProps)(reduxForm({
    ...
    onSubmit: (values, dispatch, props) => {
        const { orderId } = props
        dispatch(reduxActions.sendAddressInfoEditor.saveSendAddressInfo({ orderId, ...values }))
    }
