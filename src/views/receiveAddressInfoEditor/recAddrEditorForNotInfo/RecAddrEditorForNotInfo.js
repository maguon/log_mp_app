...
const RecAddrEditorForNotInfo = props => {
    const { recAddrEditorForNotInfoReducer: { receiveAddressInfo: { isResultStatus } } } = props
    return (
        <Container>
            <Content>
                <Field
...
const mapStateToProps = (state, ownProps) => {
    ...
    return {
        ...
        recAddrEditorForNotInfoReducer: state.recAddrEditorForNotInfoReducer
    }
}

export default connect(mapStateToProps)(reduxForm({
    form: 'recAddrForNIForm',
    onSubmit: (values, dispatch, props) => {
        const { orderId } = props
        dispatch(reduxActions.recAddrEditorForNotInfo.receiveAddressInfo({ orderId, ...values }))
    }
