...
const ReceiveAddressInfoEditor = props => {
    const { receiveAddressInfoEditorReducer: { receiveSendAddressInfo: { isResultStatus } } } = props
    return (
        <Container>
            <Content>
                <Field
...
const mapStateToProps = (state, ownProps) => {
    ...
    return {
        ...
        receiveAddressInfoEditorReducer: state.receiveAddressInfoEditorReducer
    }
}

export default connect(mapStateToProps)(reduxForm({
    form: 'receiveAddressForm',
    onSubmit: (values, dispatch, props) => {
        const { orderId } = props
        dispatch(reduxActions.receiveAddressInfoEditor.receiveSendAddressInfo({ orderId, ...values }))
    }
